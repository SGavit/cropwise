...
type MapsConfig = { apiKey: string };

function getMapsConfig(): MapsConfig {
  if (!ENV.googleMapsApiKey) throw new Error("GOOGLE_MAPS_API_KEY is not configured");
...
export async function makeRequest<T = unknown>(endpoint: string, params: Record<string, unknown> = {}, options: RequestOptions = {}): Promise<T> {
  ...
  if (!response.ok) throw new Error(`Google Maps API request failed (${response.status}): ${await response.text()}`);
